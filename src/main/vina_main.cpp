/*

   Copyright (c) 2006-2010, The Scripps Research Institute

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.

   Author: Dr. Oleg Trott <ot14@columbia.edu>, 
           The Olson Lab, 
           The Scripps Research Institute

 */

#include "vina_main.h"

/****************add by ly******************/
#include "taskq.h"
#include "job_pool.h"
#include "job.h"
#include <string.h>
#include <sstream>

#include <iostream>
#include <stdlib.h>



using boost::filesystem::path;
//#define YL_DEBUG

extern unsigned num_cpus;

void unhandle() {
	std::cout << "unhandled case\n";
	exit(1);
}

void test_msg(const std::string& str) {
	std::cout << str << std::endl;
}


path make_path(const std::string& str) {
	//return path(str, boost::filesystem::native);
	return path(str);
}

void doing(int verbosity, const std::string& str, tee& log) {
	if(verbosity > 1) {
		log << str << std::string(" ... ");
		log.flush();
	}
}

void done(int verbosity, tee& log) {
	if(verbosity > 1) {
		log << "done.";
		log.endl();
	}
}


void write_all_output(model& m, const output_container& out, sz how_many,
		const std::string& output_name, const std::string& output_path,
		const std::vector<std::string>& remarks) {
	if(out.size() < how_many)
		how_many = out.size();
	VINA_CHECK(how_many <= remarks.size());
	//ofile f(make_path(output_name));
	ofile f(make_path(output_path + output_name));
	VINA_FOR(i, how_many) {
		m.set(out[i].c);
		m.write_model(f, i+1, remarks[i]); // so that model numbers start with 1
	}
}

void do_randomization(model& m,
		const std::string& out_name,
		const vec& corner1, const vec& corner2, int seed, int verbosity, tee& log) {
	conf init_conf = m.get_initial_conf();
	rng generator(static_cast<rng::result_type>(seed));
	if(verbosity > 1) {
		log << "Using random seed: " << seed;
		log.endl();
	}
	const sz attempts = 10000;
	conf best_conf = init_conf;
	fl best_clash_penalty = 0;
	VINA_FOR(i, attempts) {
		conf c = init_conf;
		c.randomize(corner1, corner2, generator);
		m.set(c);
		fl penalty = m.clash_penalty();
		if(i == 0 || penalty < best_clash_penalty) {
			best_conf = c;
			best_clash_penalty = penalty;
		}
	}
	m.set(best_conf);
	if(verbosity > 1) {
		log << "Clash penalty: " << best_clash_penalty; // FIXME rm?
		log.endl();
	}
	m.write_structure(make_path(out_name));
}

void refine_structure(model& m, const precalculate& prec, non_cache& nc, output_type& out, const vec& cap, sz max_steps = 1000) {
	change g(m.get_size());
	quasi_newton quasi_newton_par;
	quasi_newton_par.max_steps = max_steps;
	const fl slope_orig = nc.slope;
	VINA_FOR(p, 5) {
		nc.slope = 100 * std::pow(10.0, 2.0*p);
		quasi_newton_par(m, prec, nc, out, g, cap);
		m.set(out.c); // just to be sure
		if(nc.within(m))
			break;
	}
	out.coords = m.get_heavy_atom_movable_coords();
	if(!nc.within(m))
		out.e = max_fl;
	nc.slope = slope_orig;
}

std::string vina_remark(fl e, fl lb, fl ub) {
	std::ostringstream remark;
	remark.setf(std::ios::fixed, std::ios::floatfield);
	remark.setf(std::ios::showpoint);
	remark << "REMARK VINA RESULT: " 
			<< std::setw(9) << std::setprecision(1) << e
			<< "  " << std::setw(9) << std::setprecision(3) << lb
			<< "  " << std::setw(9) << std::setprecision(3) << ub
			<< '\n';
	return remark.str();
}

output_container remove_redundant(const output_container& in, fl min_rmsd) {
	output_container tmp;
	VINA_FOR_IN(i, in)
	add_to_output_container(tmp, in[i], min_rmsd, in.size());
	return tmp;
}

model parse_bundle(const std::string& rigid_name, const boost::optional<std::string>& flex_name_opt, const std::vector<std::string>& ligand_names) {
	model tmp = (flex_name_opt) ? parse_receptor_pdbqt(make_path(rigid_name), make_path(flex_name_opt.get()))
			: parse_receptor_pdbqt(make_path(rigid_name));
	VINA_FOR_IN(i, ligand_names)
	tmp.append(parse_ligand_pdbqt(make_path(ligand_names[i])));
	return tmp;
}

model parse_bundle(const std::vector<std::string>& ligand_names) {
	VINA_CHECK(!ligand_names.empty()); // FIXME check elsewhere
	model tmp = parse_ligand_pdbqt(make_path(ligand_names[0]));
	VINA_RANGE(i, 1, ligand_names.size())
	tmp.append(parse_ligand_pdbqt(make_path(ligand_names[i])));
	return tmp;
}

model parse_bundle(const boost::optional<std::string>& rigid_name_opt, const boost::optional<std::string>& flex_name_opt, const std::vector<std::string>& ligand_names) {
	if(rigid_name_opt)
		return parse_bundle(rigid_name_opt.get(), flex_name_opt, ligand_names);
	else
		return parse_bundle(ligand_names);
}

void gen_model(std::vector<model>& models, const boost::optional<std::string>& rigid_name_opt, const boost::optional<std::string>& flex_name_opt, const strv& ligand_names, int offset, int num) {
	/***********add by ly*************/

	//VINA_FOR_IN(i, ligand_names) {
	//	model tmp = parse_bundle(rigid_name_opt, flex_name_opt, std::vector<std::string>(1, ligand_names[i]));
	//	models.push_back(tmp);
	//}

	VINA_I_RANGE(i, offset, offset + num) {
		model tmp = parse_bundle(rigid_name_opt, flex_name_opt, strv(1, ligand_names[i]));
		models.push_back(tmp);
	}

}
struct pre_para {

	pre_para(job_pool& pool_, task_q& queue_, strv& lig_list_, boost::optional<std::string>& rigid_name_, boost::optional<std::string>& flex_name_, tee& log_,
			bool score_only_, bool randomize_only_, bool local_only_, std::string out_name_,
			int verbosity_, int seed_, int exhaustiveness_, int cpu_,
			grid_dims& gd_, flv& weights_, fl energy_range_, sz max_modes_sz_, int worker_num_) : pool(pool_), queue(queue_), lig_list(lig_list_), rigid_name(rigid_name_), flex_name(flex_name_), log(log_),
					score_only(score_only_), randomize_only(randomize_only_), local_only(local_only_), out_name(out_name_), verbosity(verbosity_), seed(seed_),
					exhaustiveness(exhaustiveness_), cpu(cpu_), gd(gd_), weights(weights_), energy_range(energy_range_), max_modes_sz(max_modes_sz_), worker_num(worker_num_) {}

	strv& lig_list;
	boost::optional<std::string>& rigid_name;
	boost::optional<std::string>& flex_name;
	tee& log;
	job_pool& pool;
//	int job_offset, job_num;
	bool score_only, randomize_only, local_only;
	std::string out_name;
	int verbosity, seed, exhaustiveness, cpu;
	grid_dims& gd;
	flv& weights;
	fl energy_range;
	sz max_modes_sz;
	task_q& queue;
	int worker_num;
};



void gen_task(job* j_ptr, task_q& task_queue, model& m, rng& generator) {

	int iter_time = j_ptr->task_num;

	VINA_FOR(i, iter_time) {
		int seed = random_int(0, 1000000, generator); // for debug
		task* t = new task(j_ptr, m, seed);
		task_queue.push(t);
		j_ptr->slaves.push_back(t);
#ifdef YL_DEBUG
		std::cout << "Generate task using seed:" << seed << std::endl;
#endif
	}

}



std::istream& operator >> (std::istream& in, config& c) {
	std::string tmp;
	in >> tmp;
	strcpy(c.buffer, tmp.c_str());
	return in;
}

std::ostream& operator << (std::ostream& out, config& c) {
	out << c.rigid_name << "," << c.ligand_name << "," << c.out_name;
	return out;
}

struct trans_para {

	trans_para(strv& ligand_list, job_pool& pool, config& cf) : ligand_list(ligand_list), pool(pool), cf(cf) {}
	strv& ligand_list;
	job_pool& pool;
	config& cf;

};


void* onmic_pre_vina(void* arg) {
	struct pre_para* para = (struct pre_para*) arg;
	task_q& task_queue = para->queue;
	strv& lig_list = para->lig_list;
	boost::optional<std::string>& rigid_name_opt = para->rigid_name;
	boost::optional<std::string>& flex_name_opt = para->flex_name;
	tee& log = para->log;
	job_pool& pool = para->pool;
//	int job_offset = para->job_offset, job_num = para->job_num;
	bool score_only = para->score_only, randomize_only = para->randomize_only, local_only = para->local_only;

	int verbosity = para->verbosity, seed = para->seed, exhaustiveness = para->exhaustiveness, cpu = para->cpu;
	grid_dims& gd = para->gd;
	flv& weights = para->weights;
	fl energy_range = para->energy_range;
	sz max_modes_sz = para->max_modes_sz;

	//VINA_FOR(i, job_num) {
	int index;
	rcv_q.pop(index);
	while(index >= 0) {
		std::string out_name = para->out_name;
		std::stringstream ss;
		//ss << job_offset + i;
		ss << index;
		out_name = ss.str() + "_" + out_name;

		doing(verbosity, "Reading input", log);
		//model m = parse_bundle(rigid_name_opt, flex_name_opt, strv(1, lig_list[i + job_offset]));
		model m = parse_bundle(rigid_name_opt, flex_name_opt, strv(1, lig_list[index]));
		done(verbosity, log);

		boost::optional<model> ref;

		doing(verbosity, "Setting up the scoring function", log);

		everything t;
		VINA_CHECK(weights.size() == 6);

		weighted_terms wt(&t, weights);
		precalculate prec(wt);
		const fl left  = 0.25;
		const fl right = 0.25;
		precalculate prec_widened(prec); prec_widened.widen(left, right);

		done(verbosity, log);

		vec corner1(gd[0].begin, gd[1].begin, gd[2].begin);
		vec corner2(gd[0].end,   gd[1].end,   gd[2].end);

		parallel_mc par;

		sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom();
		par.mc.num_steps = unsigned(70 * 3 * (50 + heuristic) / 2); // 2 * 70 -> 8 * 20 // FIXME
		par.mc.ssd_par.evals = unsigned((25 + m.num_movable_atoms()) / 3);
		par.mc.min_rmsd = 1.0;
		par.mc.num_saved_mins = 20;
		par.mc.hunt_cap = vec(10, 10, 10);
		par.num_tasks = exhaustiveness;
		par.num_threads = cpu;
		par.display_progress = (verbosity > 1);
		std::cout<<"m.num_movable_atoms: "<<m.num_movable_atoms()<<std::endl;
		std::cout<<"m.get_size().num_degrees_of_freedom(): "<<m.get_size().num_degrees_of_freedom()<<std::endl;
		std::cout<<"num_steps: "<<par.mc.num_steps<<std::endl;
		std::cout<<"heuristic: "<<heuristic<<std::endl;

		const fl slope = 1e6;

		if(randomize_only) {
			unhandle();
		}
		else {
			//non_cache nc        (m, gd, &prec,         slope); // if gd has 0 n's, this will not constrain anything
			//non_cache nc_widened(m, gd, &prec_widened, slope); // if gd has 0 n's, this will not constrain anything



			bool cache_needed = !(score_only || randomize_only || local_only);

			if(cache_needed) doing(verbosity, "Analyzing the binding site", log);

			cache c("scoring_function_version001", gd, slope, atom_type::XS);

			if(cache_needed) c.populate(m, prec, m.get_movable_atom_types(prec.atom_typing_used()));

			if(cache_needed) done(verbosity, log);

			conf_size s = m.get_size();
			conf cf = m.get_initial_conf();
			fl e = max_fl;
			const vec authentic_v(1000, 1000, 1000);

			if(score_only) {
				unhandle();
			}
			else if(local_only) {
				unhandle();
			}
			else {
				rng generator(static_cast<rng::result_type>(seed));
				log << "Using random seed: " << seed;
				log.endl();
				//output_container out_cont;

				doing(verbosity, "Generating Jobs", log);
				//parallel_progress* pp;
				struct job* j_ptr;
				/*
				if(par.display_progress) {
					pp = new parallel_progress();
					j_ptr = new job(m, authentic_v, max_modes_sz, energy_range, exhaustiveness, lig_list[i + job_offset], rigid_name_opt.get(), out_name, par, &prec, &c, &prec, &c, &corner1, &corner2, pp);
				} else
					j_ptr = new job(m, authentic_v, max_modes_sz, energy_range, exhaustiveness, lig_list[i + job_offset], rigid_name_opt.get(), out_name, par, &prec, &c, &prec, &c, &corner1, &corner2, NULL);
				*/

				j_ptr = new job(m, authentic_v, max_modes_sz, energy_range, exhaustiveness, lig_list[index], rigid_name_opt.get(), out_name, par, &prec, &c, &prec, &c, &corner1, &corner2, NULL);
				done(verbosity, log);

				doing(verbosity, "Generating tasks", log);

				gen_task(j_ptr, task_queue, m, generator);

				done(verbosity, log);
			}
		}
		rcv_q.pop(index);
	}
}

void* pre_vina(void* arg) {

	struct pre_para* para = (struct pre_para*) arg;
	task_q& task_queue = para->queue;
	strv& lig_list = para->lig_list;
	boost::optional<std::string>& rigid_name_opt = para->rigid_name;
	boost::optional<std::string>& flex_name_opt = para->flex_name;
	tee& log = para->log;
	job_pool& pool = para->pool;
//	int job_offset = para->job_offset, job_num = para->job_num;
	bool score_only = para->score_only, randomize_only = para->randomize_only, local_only = para->local_only;

	int verbosity = para->verbosity, seed = para->seed, exhaustiveness = para->exhaustiveness, cpu = para->cpu;
	grid_dims& gd = para->gd;
	flv& weights = para->weights;
	fl energy_range = para->energy_range;
	sz max_modes_sz = para->max_modes_sz;

	//VINA_FOR(i, job_num) {
	int index = 0;
	while((index = pool.get_job(true)) != -1) {
		std::string out_name = para->out_name;
		std::stringstream ss;
		//ss << job_offset + i;
		ss << index;
		out_name = ss.str() + "_" + out_name;

		doing(verbosity, "Reading input", log);
		//model m = parse_bundle(rigid_name_opt, flex_name_opt, strv(1, lig_list[i + job_offset]));
		model m = parse_bundle(rigid_name_opt, flex_name_opt, strv(1, lig_list[index]));
		done(verbosity, log);

		boost::optional<model> ref;

		doing(verbosity, "Setting up the scoring function", log);

		everything t;
		VINA_CHECK(weights.size() == 6);

		weighted_terms wt(&t, weights);
		precalculate prec(wt);
		const fl left  = 0.25;
		const fl right = 0.25;
		precalculate prec_widened(prec); prec_widened.widen(left, right);

		done(verbosity, log);

		vec corner1(gd[0].begin, gd[1].begin, gd[2].begin);
		vec corner2(gd[0].end,   gd[1].end,   gd[2].end);

		parallel_mc par;

		sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom();
		par.mc.num_steps = unsigned(70 * 3 * (50 + heuristic) / 2); // 2 * 70 -> 8 * 20 // FIXME
		par.mc.ssd_par.evals = unsigned((25 + m.num_movable_atoms()) / 3);
		par.mc.min_rmsd = 1.0;
		par.mc.num_saved_mins = 20;
		par.mc.hunt_cap = vec(10, 10, 10);
		par.num_tasks = exhaustiveness;
		par.num_threads = cpu;
		par.display_progress = (verbosity > 1);
		std::cout<<"m.num_movable_atoms: "<<m.num_movable_atoms()<<std::endl;
		std::cout<<"m.get_size().num_degrees_of_freedom(): "<<m.get_size().num_degrees_of_freedom()<<std::endl;
		std::cout<<"num_steps: "<<par.mc.num_steps<<std::endl;
		std::cout<<"heuristic: "<<heuristic<<std::endl;

		const fl slope = 1e6;

		if(randomize_only) {
			unhandle();
		}
		else {
			//non_cache nc        (m, gd, &prec,         slope); // if gd has 0 n's, this will not constrain anything
			//non_cache nc_widened(m, gd, &prec_widened, slope); // if gd has 0 n's, this will not constrain anything



			bool cache_needed = !(score_only || randomize_only || local_only);

			if(cache_needed) doing(verbosity, "Analyzing the binding site", log);

			cache c("scoring_function_version001", gd, slope, atom_type::XS);

			if(cache_needed) c.populate(m, prec, m.get_movable_atom_types(prec.atom_typing_used()));

			if(cache_needed) done(verbosity, log);

			conf_size s = m.get_size();
			conf cf = m.get_initial_conf();
			fl e = max_fl;
			const vec authentic_v(1000, 1000, 1000);

			if(score_only) {
				unhandle();
			}
			else if(local_only) {
				unhandle();
			}
			else {
				rng generator(static_cast<rng::result_type>(seed));
				log << "Using random seed: " << seed;
				log.endl();
				//output_container out_cont;

				doing(verbosity, "Generating Jobs", log);
				//parallel_progress* pp;
				struct job* j_ptr;
				/*
				if(par.display_progress) {
					pp = new parallel_progress();
					j_ptr = new job(m, authentic_v, max_modes_sz, energy_range, exhaustiveness, lig_list[i + job_offset], rigid_name_opt.get(), out_name, par, &prec, &c, &prec, &c, &corner1, &corner2, pp);
				} else
					j_ptr = new job(m, authentic_v, max_modes_sz, energy_range, exhaustiveness, lig_list[i + job_offset], rigid_name_opt.get(), out_name, par, &prec, &c, &prec, &c, &corner1, &corner2, NULL);
				*/

				j_ptr = new job(m, authentic_v, max_modes_sz, energy_range, exhaustiveness, lig_list[index], rigid_name_opt.get(), out_name, par, &prec, &c, &prec, &c, &corner1, &corner2, NULL);
				done(verbosity, log);

				doing(verbosity, "Generating tasks", log);

				gen_task(j_ptr, task_queue, m, generator);

				done(verbosity, log);

#ifdef YL_DEBUG
				test_msg("Testing data now");

				test_msg("Testing job");

				test_msg("Job ligand and receptor are");
				test_msg(j_ptr->lig_name + " " + j_ptr->rcp_name);
				test_msg("Job have task number:");
				std::cout << j_ptr->task_num << std::endl;

				test_msg("Test member data");
				vec* test = &(j_ptr->data.corner1);
				if(test != NULL)
					test_msg("Fine");
				else
					test_msg("Oops! Error");

				test_msg("Testing tasks");
				task* tmp;
				task_queue.pop(tmp);
				test_msg("My owner is");
				std::cout << tmp->owner->lig_name << std::endl;
				test_msg("Testing member par_task");
				model* testm = &(tmp->par_task->m);
				if(test != NULL)
					test_msg("Fine");
				else
					test_msg("Oops! Error");
#endif
			}

		}
	}

	int worker_num = para->worker_num;
	VINA_FOR(i, worker_num)
		task_queue.push(NULL);

#ifdef YL_DEBUG
	std::cout << "Total task number is " << task_queue.size() << std::endl;

	task* t;
	task_queue.pop(t);

	test_msg("Do search");
	t->owner->par_aux(*(t->par_task));
	test_msg("Done!");
#endif

	return NULL;
}

void merge_output(job* j_ptr) {
	int iter_time = j_ptr->task_num;
	task* slave;
	VINA_FOR(i, iter_time) {
		slave = j_ptr->slaves[i];
		merge_output_containers(slave->par_task->out, j_ptr->out, 2, j_ptr->par.mc.num_saved_mins); //FIXME? min_rmsd = 2 ??

		//delete slave->par_task;
		delete slave;
	}

	j_ptr->out.sort();
}

struct do_para {
	do_para(task_q& queue_, tee& log_, int verbosity_, grid_dims& gd_, flv& weights_, std::string& out_path_) : task_queue(queue_), log(log_), verbosity(verbosity_), gd(gd_), weights(weights_), out_path(out_path_) {}
	task_q& task_queue;
	tee& log;
	int verbosity;
	grid_dims& gd;
	flv& weights;
	std::string& out_path;
};

void test_out(job* j) { //use for debug
	std::cout << "****** " << j->lig_name << "*******\n";
	VINA_FOR_IN(i, j->out)
		std::cout << j->out[i].e << std::endl;
}

void* do_vina(void* arg) {
	do_para* para = (do_para*) arg;

	task_q& task_queue = para->task_queue;
	tee& log = para->log;
	int verbosity = para->verbosity;
	grid_dims& gd = para->gd;
	flv& weights = para->weights;
	std::string out_path = para->out_path;
	while(true) {
		task* t;
		task_queue.pop(t);

		if(t == NULL)
			break;

		doing(verbosity, "Performing search", log);
		t->owner->par_aux(*(t->par_task));
		done(verbosity, log);

		int last_one = t->owner->task_done();
		if(last_one == RMN_TASK)
			continue;

		//do clean up and write output
		job* j = t->owner;
		doing(verbosity, "Merge searching output", log);
		merge_output(j);
		done(verbosity, log);

		//test_out(j);

		doing(verbosity, "Generating necessary data", log);
		//following data must be re-generated. copying them will cause unexpected behavior
		everything et;
		weighted_terms wt(&et, weights);
		precalculate prec(wt);
		const fl slope = 1e6;
		non_cache nc(j->e_data.m, gd, &prec, slope);
		done(verbosity, log);

		doing(verbosity, "Refining results", log);
		VINA_FOR_IN(i, j->out)
			refine_structure(j->e_data.m,  prec, nc, j->out[i], j->e_data.authentic_v, j->par.mc.ssd_par.evals); //FIXME? bug? is the passed parameter right
		//test_out(j);
		if(!(j->out.empty())) {
			j->out.sort();
			const fl best_mode_intramolecular_energy = j->e_data.m.eval_intramolecular(prec, j->e_data.authentic_v, j->out[0].c);
			VINA_FOR_IN(i, j->out)
				if(not_max(j->out[i].e))
					j->out[i].e = j->e_data.m.eval_adjusted(wt, prec, nc, j->e_data.authentic_v, j->out[i].c, best_mode_intramolecular_energy);
			j->out.sort();
		}

		const fl out_min_rmsd = 1;
		j->out = remove_redundant(j->out, out_min_rmsd);

		done(verbosity, log);
		//test_out(j);
		log.setf(std::ios::fixed, std::ios::floatfield);
		log.setf(std::ios::showpoint);
		log << "Ligand name: "<< j->lig_name << " Receptor name: " << j->rcp_name << "\n";
		log << '\n';
		log << "mode |   affinity | dist from best mode\n";
		log << "     | (kcal/mol) | rmsd l.b.| rmsd u.b.\n";
		log << "-----+------------+----------+----------\n";

		model best_mode_model = j->e_data.m;
		if(!(j->out.empty()))
			best_mode_model.set(j->out.front().c);

		sz how_many = 0;
		strv remarks;
		VINA_FOR_IN(i, j->out) {
			if(how_many >= j->e_data.max_modes_sz || !not_max(j->out[i].e) || j->out[i].e > j->out[0].e + j->e_data.energy_range) break; // check energy_range sanity FIXME
			++how_many;
			log << std::setw(4) << i+1
					<< "    " << std::setw(9) << std::setprecision(1) << j->out[i].e; // intermolecular_energies[i];
			j->e_data.m.set(j->out[i].c);
			//const model& r = ref ? ref.get() : best_mode_model; FIXME? BUG?
			const model& r = best_mode_model;
			const fl lb = j->e_data.m.rmsd_lower_bound(r);
			const fl ub = j->e_data.m.rmsd_upper_bound(r);
			log << "  " << std::setw(9) << std::setprecision(3) << lb
					<< "  " << std::setw(9) << std::setprecision(3) << ub; // FIXME need user-readable error messages in case of failures

			remarks.push_back(vina_remark(j->out[i].e, lb, ub));
			log.endl();
		}

		doing(verbosity, "Writing output", log);
		write_all_output(j->e_data.m, j->out, how_many, j->out_name, out_path, remarks);
		done(verbosity, log);

		log << "\nOutput will be " << j->out_name;
		log.endl();

		delete j;

		if(how_many < 1) {
			log << "WARNING: Could not find any conformations completely within the search space.\n"
					<< "WARNING: Check that it is large enough for all movable atoms, including those in the flexible side chains.";
			log.endl();
		}

	}

	return NULL;
}

void* onmic_do_vina(void* arg) {
	do_para* para = (do_para*) arg;

	task_q& task_queue = para->task_queue;
	tee& log = para->log;
	int verbosity = para->verbosity;
	grid_dims& gd = para->gd;
	flv& weights = para->weights;
	std::string out_path = para->out_path;
	while(true) {
		task* t;
		task_queue.pop(t);

		if(t == NULL)
			break;

		doing(verbosity, "Performing search", log);
		t->owner->par_aux(*(t->par_task));
		done(verbosity, log);

		int last_one = t->owner->task_done();
		if(last_one == RMN_TASK)
			continue;

		//do clean up and write output
		job* j = t->owner;
		doing(verbosity, "Merge searching output", log);
		merge_output(j);
		done(verbosity, log);

		//test_out(j);

		doing(verbosity, "Generating necessary data", log);
		//following data must be re-generated. copying them will cause unexpected behavior
		everything et;
		weighted_terms wt(&et, weights);
		precalculate prec(wt);
		const fl slope = 1e6;
		non_cache nc(j->e_data.m, gd, &prec, slope);
		done(verbosity, log);

		doing(verbosity, "Refining results", log);
		VINA_FOR_IN(i, j->out)
			refine_structure(j->e_data.m,  prec, nc, j->out[i], j->e_data.authentic_v, j->par.mc.ssd_par.evals); //FIXME? bug? is the passed parameter right
		//test_out(j);
		if(!(j->out.empty())) {
			j->out.sort();
			const fl best_mode_intramolecular_energy = j->e_data.m.eval_intramolecular(prec, j->e_data.authentic_v, j->out[0].c);
			VINA_FOR_IN(i, j->out)
				if(not_max(j->out[i].e))
					j->out[i].e = j->e_data.m.eval_adjusted(wt, prec, nc, j->e_data.authentic_v, j->out[i].c, best_mode_intramolecular_energy);
			j->out.sort();
		}

		const fl out_min_rmsd = 1;
		j->out = remove_redundant(j->out, out_min_rmsd);

		done(verbosity, log);
		//test_out(j);
		log.setf(std::ios::fixed, std::ios::floatfield);
		log.setf(std::ios::showpoint);
		log << "Ligand name: "<< j->lig_name << " Receptor name: " << j->rcp_name << "\n";
		log << '\n';
		log << "mode |   affinity | dist from best mode\n";
		log << "     | (kcal/mol) | rmsd l.b.| rmsd u.b.\n";
		log << "-----+------------+----------+----------\n";

		model best_mode_model = j->e_data.m;
		if(!(j->out.empty()))
			best_mode_model.set(j->out.front().c);

		sz how_many = 0;
		strv remarks;
		VINA_FOR_IN(i, j->out) {
			if(how_many >= j->e_data.max_modes_sz || !not_max(j->out[i].e) || j->out[i].e > j->out[0].e + j->e_data.energy_range) break; // check energy_range sanity FIXME
			++how_many;
			log << std::setw(4) << i+1
					<< "    " << std::setw(9) << std::setprecision(1) << j->out[i].e; // intermolecular_energies[i];
			j->e_data.m.set(j->out[i].c);
			//const model& r = ref ? ref.get() : best_mode_model; FIXME? BUG?
			const model& r = best_mode_model;
			const fl lb = j->e_data.m.rmsd_lower_bound(r);
			const fl ub = j->e_data.m.rmsd_upper_bound(r);
			log << "  " << std::setw(9) << std::setprecision(3) << lb
					<< "  " << std::setw(9) << std::setprecision(3) << ub; // FIXME need user-readable error messages in case of failures

			remarks.push_back(vina_remark(j->out[i].e, lb, ub));
			log.endl();
		}

		doing(verbosity, "Writing output", log);
		write_all_output(j->e_data.m, j->out, how_many, j->out_name, out_path, remarks);
		done(verbosity, log);

		log << "\nOutput will be " << j->out_name;
		log.endl();

		delete j;

		if(how_many < 1) {
			log << "WARNING: Could not find any conformations completely within the search space.\n"
					<< "WARNING: Check that it is large enough for all movable atoms, including those in the flexible side chains.";
			log.endl();
		}

		done_q.push(1);

	}

	return NULL;
}

vina_main_t *global_vina_main;

#ifndef __MIC__
	tbb::concurrent_bounded_queue<int> *rcv_q;
	tbb::concurrent_bounded_queue<int> *done_q;
#else
	tbb::concurrent_bounded_queue<int> *slot_q;
#endif

#define RCD_TIME

#ifdef RCD_TIME
#include <sys/time.h>

class time_rcd {

public:
	double get_duration() const {
		return duration;
	}

	void start() {
		gettimeofday(&tv, NULL);
		begin = tv.tv_sec*1000*1000 + tv.tv_usec;
	}

	void stop() {
		gettimeofday(&tv, NULL);
		end = tv.tv_sec*1000*1000 + tv.tv_usec;
		duration = (double)(end - begin) / 1000 / 1000;
	}

private:
	struct timeval tv;
	long long begin, end;
	double duration;

};

time_rcd timer;
#endif

using boost::filesystem::path;

unsigned num_cpus = boost::thread::hardware_concurrency();

std::string default_output(const std::string& input_name) {
	std::string tmp = input_name;
	if(tmp.size() >= 6 && tmp.substr(tmp.size()-6, 6) == ".pdbqt")
		tmp.resize(tmp.size() - 6); // FIXME?
	return tmp + "_out.pdbqt";
}

void parse_ligand(std::string& file, strv& lig_list) {
	//add by ly
	std::ifstream in_file;
	try {
		in_file.open(file.c_str());
	}
	catch(...) {
		std::cout << "cannot open file" << std::endl;
	}

	std::string lig_file;

	while(in_file) {
		std::getline(in_file, lig_file);
		if(lig_file.size() > 0)
			lig_list.push_back(lig_file);

		/*		std::vector<std::string> names;
		tokenize(lig_file, names); //TODO
		if(names.size() > 0) {
			lig_list.push_back(names[0]);
		}*/


	}
}

struct usage_error : public std::runtime_error {
	usage_error(const std::string& message) : std::runtime_error(message) {}
};

struct options_occurrence {
	bool some;
	bool all;
	options_occurrence() : some(false), all(true) {} // convenience
	options_occurrence& operator+=(const options_occurrence& x) {
		some = some || x.some;
		all  = all  && x.all;
		return *this;
	}
};

options_occurrence get_occurrence(boost::program_options::variables_map& vm, boost::program_options::options_description& d) {
	options_occurrence tmp;
	VINA_FOR_IN(i, d.options())
	if(vm.count((*d.options()[i]).long_name()))
		tmp.some = true;
	else
		tmp.all = false;
	return tmp;
}

void check_occurrence(boost::program_options::variables_map& vm, boost::program_options::options_description& d) {
	VINA_FOR_IN(i, d.options()) {
		const std::string& str = (*d.options()[i]).long_name();
		if(!vm.count(str))
			std::cerr << "Required parameter --" << str << " is missing!\n";
	}
}




void reporterr() {
	std::cout << "out of the range \n";	
}

void serial_strings(char *buf, int len, int argc, char **argv) {
	int left = len;
	
	if (left < sizeof(int)) reporterr();
	int* iptr = reinterpret_cast<int*>(buf);
	iptr[0] = argc;

	buf += sizeof(int);
	left -= sizeof(int);

	int i;
	for (i = 0; i < argc; i++) {
		int slen = strlen(argv[i]) + 1;
		if (left < slen) reporterr();

		memcpy(buf, argv[i], slen);

		buf += slen;
		left -= slen;
	}
	
}

void deserial_strings(char *buf, int len, int &argc, char** &argv) {
	int* iptr = reinterpret_cast<int*>(buf);
	argc = iptr[0];

	if (argc == 0) {
		argv = NULL;
		return;
	}

	buf += sizeof(int);
	
	argv = (char **)malloc(sizeof(char *) * argc);

	int i;
	for (i = 0; i < argc; i++) {
		int slen = strlen(buf) + 1;
		argv[i] = (char *) malloc(sizeof(char) * slen);
		memcpy(argv[i], buf, slen);

		buf += slen;
	}
}

int boot_up(int argc, char* argv[]) {
	using namespace boost::program_options;
	const std::string version_string = "AutoDock Vina 1.1.2 (May 11, 2011)";
	const std::string error_message = "\n\n\
	Please contact the author, Dr. Oleg Trott <ot14@columbia.edu>, so\n\
	that this problem can be resolved. The reproducibility of the\n\
	error may be vital, so please remember to include the following in\n\
	your problem report:\n\
	* the EXACT error message,\n\
	* your version of the program,\n\
	* the type of computer system you are running it on,\n\
	* all command line options,\n\
	* configuration file (if used),\n\
	* ligand file as PDBQT,\n\
	* receptor file as PDBQT,\n\
	* flexible side chains file as PDBQT (if used),\n\
	* output file as PDBQT (if any),\n\
	* input (if possible),\n\
	* random seed the program used (this is printed when the program starts).\n\
	\n\
	Thank you!\n";

	const std::string cite_message = "\
	#################################################################\n\
	# If you used AutoDock Vina in your work, please cite:          #\n\
	#                                                               #\n\
	# O. Trott, A. J. Olson,                                        #\n\
	# AutoDock Vina: improving the speed and accuracy of docking    #\n\
	# with a new scoring function, efficient optimization and       #\n\
	# multithreading, Journal of Computational Chemistry 31 (2010)  #\n\
	# 455-461                                                       #\n\
	#                                                               #\n\
	# DOI 10.1002/jcc.21334                                         #\n\
	#                                                               #\n\
	# Please see http://vina.scripps.edu for more information.      #\n\
	#################################################################\n";
	std::string liglist_file;
	strv lig_list;
	try {

#ifdef RCD_TIME
		timer.start();
#endif

		std::string rigid_name, ligand_name, flex_name, config_name, out_name, log_name, out_path;
		fl center_x, center_y, center_z, size_x, size_y, size_z;
		int cpu = 0, seed, exhaustiveness, verbosity = 2, num_modes = 9;
		int num_workers = num_cpus - 1;
		fl energy_range = 2.0;
		//unsigned num_cpus = boost::thread::hardware_concurrency();

		// -0.035579, -0.005156, 0.840245, -0.035069, -0.587439, 0.05846
		fl weight_gauss1      = -0.035579;
		fl weight_gauss2      = -0.005156;
		fl weight_repulsion   =  0.840245;
		fl weight_hydrophobic = -0.035069;
		fl weight_hydrogen    = -0.587439;
		fl weight_rot         =  0.05846;
		bool score_only = false, local_only = false, randomize_only = false, help = false, help_advanced = false, version = false; // FIXME

		positional_options_description positional; // remains empty

		options_description inputs("Input");
		inputs.add_options()
									("receptor", value<std::string>(&rigid_name), "rigid part of the receptor (PDBQT)")
									("flex", value<std::string>(&flex_name), "flexible side chains, if any (PDBQT)")
									("ligand", value<std::string>(&ligand_name), "ligand (PDBQT)")
									("lig_list", value<std::string>(&liglist_file), "file specifies ligand list")
									;
		//options_description search_area("Search area (required, except with --score_only)");
		options_description search_area("Search space (required)");
		search_area.add_options()
									("center_x", value<fl>(&center_x), "X coordinate of the center")
									("center_y", value<fl>(&center_y), "Y coordinate of the center")
									("center_z", value<fl>(&center_z), "Z coordinate of the center")
									("size_x", value<fl>(&size_x), "size in the X dimension (Angstroms)")
									("size_y", value<fl>(&size_y), "size in the Y dimension (Angstroms)")
									("size_z", value<fl>(&size_z), "size in the Z dimension (Angstroms)")
									;
		//options_description outputs("Output prefixes (optional - by default, input names are stripped of .pdbqt\nare used as prefixes. _001.pdbqt, _002.pdbqt, etc. are appended to the prefixes to produce the output names");
		options_description outputs("Output (optional)");
		outputs.add_options()
									("out", value<std::string>(&out_name), "output models (PDBQT), the default is chosen based on the ligand file name")
									("log", value<std::string>(&log_name), "optionally, write log file")
									("out_path", value<std::string>(&out_path), "output path")
									;
		options_description advanced("Advanced options (see the manual)");
		advanced.add_options()
									("score_only",     bool_switch(&score_only),     "score only - search space can be omitted")
									("local_only",     bool_switch(&local_only),     "do local search only")
									("randomize_only", bool_switch(&randomize_only), "randomize input, attempting to avoid clashes")
									("weight_gauss1", value<fl>(&weight_gauss1)->default_value(weight_gauss1),                "gauss_1 weight")
									("weight_gauss2", value<fl>(&weight_gauss2)->default_value(weight_gauss2),                "gauss_2 weight")
									("weight_repulsion", value<fl>(&weight_repulsion)->default_value(weight_repulsion),       "repulsion weight")
									("weight_hydrophobic", value<fl>(&weight_hydrophobic)->default_value(weight_hydrophobic), "hydrophobic weight")
									("weight_hydrogen", value<fl>(&weight_hydrogen)->default_value(weight_hydrogen),          "Hydrogen bond weight")
									("weight_rot", value<fl>(&weight_rot)->default_value(weight_rot),                         "N_rot weight")
									;
		options_description misc("Misc (optional)");
		misc.add_options()
									("cpu", value<int>(&cpu), "the number of CPUs to use (the default is to try to detect the number of CPUs or, failing that, use 1)")
									("num_workers", value<int>(&num_workers), "the number of total workers")
									("seed", value<int>(&seed), "explicit random seed")
									("exhaustiveness", value<int>(&exhaustiveness)->default_value(8), "exhaustiveness of the global search (roughly proportional to time): 1+")
									("num_modes", value<int>(&num_modes)->default_value(9), "maximum number of binding modes to generate")
									("energy_range", value<fl>(&energy_range)->default_value(3.0), "maximum energy difference between the best binding mode and the worst one displayed (kcal/mol)")
									;
		options_description config("Configuration file (optional)");
		config.add_options()
									("config", value<std::string>(&config_name), "the above options can be put here")
									;
		options_description info("Information (optional)");
		info.add_options()
									("help",          bool_switch(&help), "display usage summary")
									("help_advanced", bool_switch(&help_advanced), "display usage summary with advanced options")
									("version",       bool_switch(&version), "display program version")
									;
		options_description desc, desc_config, desc_simple;
		desc       .add(inputs).add(search_area).add(outputs).add(advanced).add(misc).add(config).add(info);
		desc_config.add(inputs).add(search_area).add(outputs).add(advanced).add(misc);
		desc_simple.add(inputs).add(search_area).add(outputs).add(misc).add(config).add(info);

		variables_map vm;
		try {
			store(command_line_parser(argc, argv)
					.options(desc)
					.style(command_line_style::default_style ^ command_line_style::allow_guessing)
					.positional(positional)
					.run(),
					vm);
			notify(vm);
		}
		catch(boost::program_options::error& e) {
			std::cerr << "Command line parse error: " << e.what() << '\n' << "\nCorrect usage:\n" << desc_simple << '\n';
			return 1;
		}
		if(vm.count("config")) {
			try {
				path name = make_path(config_name);
				ifile config_stream(name);
				store(parse_config_file(config_stream, desc_config), vm);
				notify(vm);
			}
			catch(boost::program_options::error& e) {
				std::cerr << "Configuration file parse error: " << e.what() << '\n' << "\nCorrect usage:\n" << desc_simple << '\n';
				return 1;
			}
		}
		if(help) {
			std::cout << desc_simple << '\n';
			return 0;
		}
		if(help_advanced) {
			std::cout << desc << '\n';
			return 0;
		}
		if(version) {
			std::cout << version_string << '\n';
			return 0;
		}

		bool search_box_needed = !score_only; // randomize_only and local_only still need the search space
		bool output_produced   = !score_only;
		bool receptor_needed   = !randomize_only;

		if(receptor_needed) {
			if(vm.count("receptor") <= 0) {
				std::cerr << "Missing receptor.\n" << "\nCorrect usage:\n" << desc_simple << '\n';
				return 1;
			}
		}

		/******modified by ly*****/
		if((vm.count("ligand") <= 0) && (vm.count("lig_list") <= 0)) {
			std::cerr << "Missing ligand.\n" << "\nCorrect usage:\n" << desc_simple << '\n';
			return 1;
		} else if(vm.count("lig_list") > 0) {
			parse_ligand(liglist_file, lig_list);
		}

		/*********end***********/
		if(cpu < 1)
			cpu = 1;
		if(vm.count("seed") == 0)
			seed = auto_seed();
		if(exhaustiveness < 1)
			throw usage_error("exhaustiveness must be 1 or greater");
		if(num_modes < 1)
			throw usage_error("num_modes must be 1 or greater");
		sz max_modes_sz = static_cast<sz>(num_modes);

		boost::optional<std::string> rigid_name_opt;
		if(vm.count("receptor"))
			rigid_name_opt = rigid_name;

		boost::optional<std::string> flex_name_opt;
		if(vm.count("flex"))
			flex_name_opt = flex_name;

		if(vm.count("flex") && !vm.count("receptor"))
			throw usage_error("Flexible side chains are not allowed without the rest of the receptor"); // that's the only way parsing works, actually

		tee log;
		if(vm.count("log") > 0)
			log.init(log_name);

		if(search_box_needed) {
			options_occurrence oo = get_occurrence(vm, search_area);
			if(!oo.all) {
				check_occurrence(vm, search_area);
				std::cerr << "\nCorrect usage:\n" << desc_simple << std::endl;
				return 1;
			}
			if(size_x <= 0 || size_y <= 0 || size_z <= 0)
				throw usage_error("Search space dimensions should be positive");
		}

		log << cite_message << '\n';

		if(search_box_needed && size_x * size_y * size_z > 27e3) {
			log << "WARNING: The search space volume > 27000 Angstrom^3 (See FAQ)\n";
		}

		if(output_produced) { // FIXME
			if(!vm.count("out")) {
				if(vm.count("ligand") > 0)
					out_name = default_output(ligand_name);
				else
					out_name = default_output(liglist_file);
				log << "Output will be " << out_name << '\n';
			}
		}

		//exit(0); //DEBUG
		grid_dims gd; // n's = 0 via default c'tor

		flv weights;
		weights.push_back(weight_gauss1);
		weights.push_back(weight_gauss2);
		weights.push_back(weight_repulsion);
		weights.push_back(weight_hydrophobic);
		weights.push_back(weight_hydrogen);
		weights.push_back(5 * weight_rot / 0.1 - 1); // linearly maps onto a different range, internally. see everything.cpp

		if(search_box_needed) {
			const fl granularity = 0.375;
			vec span(size_x,   size_y,   size_z);
			vec center(center_x, center_y, center_z);
			VINA_FOR_IN(i, gd) {
				gd[i].n = sz(std::ceil(span[i] / granularity));
				fl real_span = granularity * gd[i].n;
				gd[i].begin = center[i] - real_span/2;
				gd[i].end = gd[i].begin + real_span;
			}
		}
		if(vm.count("cpu") == 0) {

			if(verbosity > 1) {
				if(num_cpus > 0)
					log << "Detected " << num_cpus << " CPU" << ((num_cpus > 1) ? "s" : "") << '\n';
				else
					log << "Could not detect the number of CPUs, using 1\n";
			}
			if(num_cpus > 0)
				cpu = num_cpus;
			else
				cpu = 1;
		}
		if(cpu < 1)
			cpu = 1;
		if(verbosity > 1 && exhaustiveness < cpu)
			log << "WARNING: at low exhaustiveness, it may be impossible to utilize all CPUs\n";

		/*************************finish command line parse*******************************/
		struct config trans(center_x, center_y, center_z, size_x, size_y, size_z, cpu, seed, exhaustiveness, verbosity, num_modes, energy_range, weight_gauss1, weight_gauss2, weight_repulsion, weight_hydrophobic, weight_hydrogen, weight_rot);
		int my_rank = 0, node_num = 1;

		int job_offset, job_num;

		#ifndef __MIC__
			//-------- cpu side ----------
			job_num = lig_list.size() / node_num;
			job_offset = job_num * my_rank;
		#endif

		vina_main(lig_list, job_offset, job_num, num_workers,
			rigid_name_opt, flex_name_opt, out_name, out_path,
			exhaustiveness, verbosity, seed, score_only, local_only, randomize_only,
			gd, weights, max_modes_sz, energy_range, cpu, log, trans);

#ifdef RCD_TIME
		timer.stop();
		std::cout << "\nTotal execution time is: " << timer.get_duration() << std::endl;
#endif

	}
	catch(file_error& e) {
		//std::cerr << "\n\nError: could not open \"" << e.name.native_file_string() << "\" for " << (e.in ? "reading" : "writing") << ".\n";
		std::cerr << "\n\nError: could not open \"" << e.name << "\" for " << (e.in ? "reading" : "writing") << ".\n";
		return 1;
	}
	catch(boost::filesystem::filesystem_error& e) {
		std::cerr << "\n\nFile system error: " << e.what() << '\n';
		return 1;
	}
	catch(usage_error& e) {
		std::cerr << "\n\nUsage error: " << e.what() << ".\n";
		return 1;
	}
	catch(parse_error& e) {
		//std::cerr << "\n\nParse error on line " << e.line << " in file \"" << e.file.native_file_string() << "\": " << e.reason << '\n';
		std::cerr << "\n\nParse error on line " << e.line << " in file \"" << e.file << "\": " << e.reason << '\n';
		return 1;
	}
	catch(std::bad_alloc&) {
		std::cerr << "\n\nError: insufficient memory!\n";
		return 1;
	}

	// Errors that shouldn't happen:

	catch(std::exception& e) {
		std::cerr << "\n\nAn error occurred: " << e.what() << ". " << error_message;
		return 1;
	}
	catch(internal_error& e) {
		std::cerr << "\n\nAn internal error occurred in " << e.file << "(" << e.line << "). " << error_message;
		return 1;
	}
	catch(...) {
		std::cerr << "\n\nAn unknown error occurred. " << error_message;
		return 1;
	}
	//vina_main(argc, argv);
	return 0;
}


 vina_main_t::vina_main_t(strv& lig_list, int job_offset, int job_num, int worker_num,
		boost::optional<std::string> &rigid_name_opt, boost::optional<std::string>& flex_name_opt, std::string& out_name, std::string& out_path,
		int exhaustiveness, int verbosity, int seed, bool score_only, bool local_only, bool randomize_only,
		grid_dims& gd, flv& weights, sz max_modes_sz, fl energy_range, int cpu, tee& log, config& trans): 
			lig_list(lig_list),
			job_offset(job_offset),
			job_num(job_num),
			worker_num(worker_num),
			rigid_name_opt(rigid_name_opt),
			flex_name_opt(flex_name_opt),
			out_name(out_name),
			out_path(out_path),
			exhaustiveness(exhaustiveness),
			verbosity(verbosity),
			seed(seed),
			score_only(score_only),
			local_only(local_only),
			randomize_only(randomize_only),
			gd(gd),
			weights(weights),
			max_modes_sz(max_modes_sz),
			energy_range(energy_range),
			cpu(cpu),
			log(log),
			trans(trans) {

		cus_tid = new pthread_t[worker_num];
	}



int vina_main(std::vector<std::string>& lig_list, int job_offset, int job_num, int num_workers,
			  boost::optional<std::string> rigid_name_opt, boost::optional<std::string> flex_name_opt, std::string& out_name, std::string& out_path,
		      int exhaustiveness, int verbosity, int seed, bool score_only, bool local_only, bool randomize_only,
			  grid_dims& gd, flv& weights, sz max_modes_sz, fl energy_range, int cpu, tee& log, config& trans) {

	trans.rigid_name = rigid_name_opt.get();
	global_vina_main = new vina_main_t(lig_list, job_offset, job_num, num_workers,
			rigid_name_opt, flex_name_opt, out_name, out_path,
			exhaustiveness, verbosity, seed, score_only, local_only, randomize_only,
			gd, weights, max_modes_sz, energy_range, cpu, log, trans);
	int ret = 0;


	//job offset & num is set only on cpu side

	#define SLOT_NUM 10

	#ifndef __MIC__ // -------------------- cpu side ----------------------------------
		slot_q = new tbb::concurrent_bounded_queue<int>;
		slot_q->set_capacity(SLOT_NUM);
		VINA_FOR(i, SLOT_NUM)
			slot_q->push(1);

		ret = global_vina_main->boot();
	#else
		//create worker flow
		// ------------------------ mic side ------------------------------------
		rcv_q = new tbb::concurrent_bounded_queue<int>;
		done_q = new tbb::concurrent_bounded_queue<int>;
		
		ret = global_vina_main->mic_boot();
	#endif

	return ret;
}
	
void* trans_index(void* arg) {
	job_pool* jp_ptr = (job_pool* ) arg;
	const char* func_name = "mic_rcv"
	int index = 0;
	while((index = jp_ptr->get_job(false)) != -1) {

		//---- get slot -----
		int tmp;
		slot_q->pop(tmp);

		//--- send index -----
		CHECK_RESULT(
		COIProcessGetFunctionHandles(
        	proc,         // Process to query for the function
        	1,            // The number of functions to query
        	&func_name,   // The name of the function
        	&func         // A handle to the function
        ));

        COIEVENT              completion_event;
        CHECK_RESULT(
    	COIPipelineRunFunction(
        	pipeline, func,    // Pipeline handle and function handle
        	0, NULL, NULL, // Buffers and access flags to pass to the function
        	0, NULL,           // Input dependencies
        	&index, sizeof(int),            // Misc data to pass to the function
        	NULL, 0,            // Return values that will be passed back
        	&completion_event   // Event to signal when the function completes
    	));

    	CHECK_RESULT(
        COIEventWait(
            1,                           // Number of events to wait for
            &completion_event,        // Event handles
            -1,                          // Wait indefinitely
            true,                        // Wait for all events
            NULL, NULL                   // Number of events signaled
                                         // and their indices
        ));
	}
}

void* wait_done(void) {
	const func_name = "wait_sig";
	int rst = 1;
	while(rst >= 0) {
		CHECK_RESULT(
		COIProcessGetFunctionHandles(
       		proc,         // Process to query for the function
    	   	1,            // The number of functions to query
       		&func_name,   // The name of the function
       		&func         // A handle to the function
       	));

		COIEVENT              completion_event;
		CHECK_RESULT(
		COIPipelineRunFunction(
       		pipeline, func,    // Pipeline handle and function handle
       		0, NULL, NULL, // Buffers and access flags to pass to the function
       		0, NULL,           // Input dependencies
       		NULL, 0,            // Misc data to pass to the function
       		&rst, sizeof(int),            // Return values that will be passed back
       		&completion_event   // Event to signal when the function completes
       	));

		CHECK_RESULT(
		COIEventWait(
        	1,                           // Number of events to wait for
        	&completion_event,        // Event handles
        	-1,                          // Wait indefinitely
        	true,                        // Wait for all uevents
        	NULL, NULL                   // Number of events signaled
        	                             // and their indices
        ));

        slot_q.push(1);
	}
}

int vina_main_t::boot() {


	job_pool pool(job_offset, job_num);
	task_q task_queue;
	task_queue.set_capacity(num_cpus * 2);
	
	pre_para para(pool, task_queue, lig_list, rigid_name_opt, flex_name_opt, log, score_only, randomize_only, 
			local_only, out_name, verbosity, seed, exhaustiveness, cpu, gd, weights, energy_range, max_modes_sz, worker_num);

	do_para d_para(task_queue, log, verbosity, gd, weights, out_path);
	//trans_para t_para(lig_list, pool, trans);

	pthread_create(&pro_tid, NULL, pre_vina, (void* ) &para);

	VINA_FOR(i, worker_num) {
		pthread_create(&cus_tid[i], NULL, do_vina, (void* ) &d_para);
	}

	//create trans & receive threads 
	pthread_create(&trans_tid, NULL, trans_index, (void* ) &pool);
	pthread_create(&wait_tid, NULL, wait_done, NULL); // TODO

	pthread_join(pro_tid, NULL);

	VINA_FOR(i, worker_num) {
		pthread_join(cus_tid[i], NULL);
	}

	pthread_join(trans_tid, NULL);
	return 0;
}

int vina_main_t::mic_boot() {
	task_q task_queue;
	task_queue.set_capacity(num_cpus * 2);

	pre_para para(pool, task_queue, lig_list, rigid_name_opt, flex_name_opt, log, score_only, randomize_only, 
			local_only, out_name, verbosity, seed, exhaustiveness, cpu, gd, weights, energy_range, max_modes_sz, worker_num);

	do_para d_para(task_queue, log, verbosity, gd, weights, out_path);

	pthread_create(&pro_tid, NULL, onmic_pre_vina, (void* ) &para);

	VINA_FOR(i, worker_num) {
		pthread_create(&cus_tid[i], NULL, onmic_do_vina, (void* ) &d_para);
	}

	pthread_join(pro_tid, NULL);

	VINA_FOR(i, worker_num) {
		pthread_join(cus_tid[i], NULL);
	}

	return 0;
}